    ...
    if(!read)
    {
        ...
        exit(0);
            ...
            while(getline(read,st))
            {
                 ...
                 //unremapPoint(row(xPoints,count));//unremap or remap won't change tha xPoint's value , it just return an unmap X point
                 
                 if(count<10)
                 {
                   std::cout<<"point after unremap is "<< row(xPoints,count)<<std::endl;
