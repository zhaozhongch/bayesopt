    ...
    for(size_t i=0; i<yPoints.size(); i++)
      {
        ...
        if(yPoints[i]>mParameters.SLAM_Fail_Threshold)
        {
          //std::cout<<"try new ini point"<<std::endl;
          //std::cout<<"SLAM failed because the parameters are too bad, ignore this iteration and find another point"<<std::endl;
          //std::cout<<"Be careful this will decrease iteration number by one"<<std::endl;
          ...
          //i=0;
          //continue;
          //yPoints[i] = mParameters.SLAM_Fail_Threshold;
        }
