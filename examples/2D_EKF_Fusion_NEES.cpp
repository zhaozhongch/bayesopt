...
//g++ 2D_EKF_Fusion_NEES.cpp -o 2D_EKF_Fusion_NEES -I /usr/include/eigen3  -lceres -lglog -lpthread -fopenmp -lcholmod -lblas -llapack -lbayesopt -lnlopt -lm
...
public:
  ...
  double evaluateSample(const vectord& xin)
  {
    ...
    system("./../../ekf_bayesopt_noupload/applications/kf1/2D_bayes_NEES");
    
    ifstream rT("/home/zhaozhong/ekf_bayesopt_noupload/applications/kf1/data/NEES.csv");
    if(!rT) {cout<<"no such file"<<endl;}
    string NEES;
    getline(rT,NEES);
    rT.close();
   
    double re = (double)atof(NEES.c_str());
...
// the l_type is not a string variable, so not need "" but the cLCB is. You can see these in parameters.hpp                          
  ...
  lowerBound(0)=0.0001;upperBound(0)=10.0;
  lowerBound(1)=0.0001;upperBound(1)=10.0;
