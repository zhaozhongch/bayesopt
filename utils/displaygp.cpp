...
namespace bayesopt
{
  ...
  {      
	...
	{ 
      ...
      n_points = bopt->getData()->getNSamples();
      for (size_t i = 0; i<n_points;++i)
	{
      ...
      state_ii = 0;    
      status = STOP;//why stop here?? Because if you see the example program it wants people press r then we run. So normally it stops, I change it to let it directly run
      //status = RUN;
    };
      ...
      if (status != NOT_READY)
	{
	  ...
	  //static int plot_flag = 0;
           ...
           //oss<<filename_first<<plot_flag<<filename_last<<std::endl;           
	  ...
	  for(size_t i=0; i<n; ++i)
	    {
	      ...
	      //z[i] = bopt_model->evaluateSample(q);              //Target function true value
              // wr<<x[i]<<","<<y[i]<<","<<su[i]<<","<<sl[i]<<","<<c[i]<<std::endl;
	    }
           //size_t sample_point_flag ;
           //for(sample_point_flag=0;sample_point_flag<(plot_flag+n_points);sample_point_flag++)
           //{
             //wr<<lx[sample_point_flag]<<","<<ly[sample_point_flag]<<std::endl;//so last several rows is sample point
           //}
           //wr.close();
	  //GP subplot
	  subplot(2,1,1);
	  ...
	  subplot(2,1,2);
	  plot(x,c); set(3);
           //save(filename.str());// this version matplot doesn't have save...CNM
           //sleep(5);
           //plot_flag++;
           //std::cout<<"plot flag is "<<plot_flag<<std::endl;
	}
         //status = RUN;
    };
      ...
      if (status != NOT_READY)
	{
           ...
           //y = linspace
           //y[i] = pd->getMean();
           std::cout<<"mean is "<< pd->getMean()<<std::endl;
