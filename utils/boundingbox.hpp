...
namespace bayesopt {
  ...
  namespace utils {
	...
	return ublas_elementwise_prod(vin,mRangeBound) + mLowerBound;
      ...
      {
         V in = vin - mLowerBound;
	return ublas_elementwise_div(in, mRangeBound); 
      }  // normalizeVector
      //original normalizeVector function is just
     // return ublas_elementwise_div(vin - mLowerBound, mRangeBound); which cannot be run, change it as above
